...
export function ParticipantTile({ participant, cameraTrackRef, isLocal }: ParticipantTileProps) {
  ...
  const hasCamera = !!cameraTrackRef;
  ...
  return (
    <div className={`flex flex-col items-center gap-2 p-3 rounded-lg transition-colors ${
      ...
      <div className={`relative ${hasCamera ? "w-full aspect-video rounded overflow-hidden bg-black" : ""}`}>
        ...
        {isMuted && (
          <div className={`${hasCamera ? "absolute bottom-1 right-1" : "absolute -bottom-1 -right-1"} w-5 h-5 bg-danger rounded-full flex items-center justify-center`}>
        ...
        {isSpeaking && !isMuted && (
          <div className={`${hasCamera ? "absolute bottom-1 right-1" : "absolute -bottom-1 -right-1"} w-5 h-5 bg-status-online rounded-full flex items-center justify-center`}>
